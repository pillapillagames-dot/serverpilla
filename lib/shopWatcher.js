...
async function creditOrder(order, signature) {
  ...
  const fresh = await db.prepare('SELECT status FROM premium_orders WHERE id = ?').get(order.id);
  if (!fresh || fresh.status !== 'pending') return;

  const tx = db.transaction(async () => {
    await db.prepare('INSERT OR IGNORE INTO player_stats (license_id) VALUES (?)').run(order.license_id);
    const stats = await db.prepare('SELECT coins FROM player_stats WHERE license_id = ?').get(order.license_id);
...
async function checkOrder(order) {
  ...
  for (const sigInfo of signatures) {
    ...
    try {
      ...
      await creditOrder(order, sigInfo.signature);
      return;
    } catch (err) {
...
function startShopWatcher() {
  setInterval(async () => {
    (await tick()).catch((err) => console.error('shopWatcher: error en el ciclo de vigilancia:', err));
  }, POLL_INTERVAL_MS);
